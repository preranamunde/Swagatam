...
const ForgotPasswordOfficerScreen = ({ navigation }) => {
  ...
  const handleSendOtp = () => {
    ...
    setOtpSent(true);
    Alert.alert('OTP Sent', `OTP: ${HARDCODED_OTP}`);
  };
  ...
  const handleResendOtp = () => {
    setOtp('');
    Alert.alert('OTP Resent', `OTP: ${HARDCODED_OTP}`);
  };
  ...
  const handleResetPassword = () => {
    ...
    Alert.alert('Success', 'Your password has been reset successfully!', [
      { text: 'Login Now', onPress: () => navigation.navigate('OfficerLogin') },
    ]);
  };
      ...
      {!otpSent ? (
        ...
        <>
          <View style={styles.otpSentBanner}>
            ...
            <TouchableOpacity onPress={() => { setOtpSent(false); setEmail(''); }}>
...
const styles = StyleSheet.create({
  ...
  govTitle: { fontSize: 22, fontWeight: '700', color: '#FFFFFF', letterSpacing: 0.5 },
  headerWave: {
    position: 'absolute',
    bottom: -1,
    left: 0,
...
export default ForgotPasswordOfficerScreen;